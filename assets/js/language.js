const Language = {
  ...
  init: function () {
   ...
   if (Settings.language !== 'en' && this.availableLanguages.includes(Settings.language)) {
...
translateDom: function (context) {
  ...
  Array.from((context || document).querySelectorAll('[data-text]')).forEach(element => {
    ...
    console.log("[translateDom] key:", transKey, "element:", element);

    let string = Language.get(transKey, element.dataset.textOptional);
    console.log("[translateDom] Language.get result:", string);

    string = string.replace(/\{([\w.]+)\}/g, (match, p1) =>
      this.allowedPlaceholders.includes(p1) ? match : '---'
    );
    console.log("[translateDom] final string:", string);
